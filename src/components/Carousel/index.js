...
export default class Carousel extends React.Component {
  constructor() {
    ...
    const { slidesToScroll, slidesToShow, cellAlign } = this.getPropsByTransitionMode(
      this.props,
  ...
  render() {
    const duration =
      this.state.dragging || this.state.resetWrapAroundPosition ? 0 : this.props.speed
      ...
      >
        <Animate
          ...
          children={({ tx, ty }) => (
            ...
            >
              <TransitionControl {...this.getTransitionProps()} deltaX={tx} deltaY={ty}>
                {validChildren}
              </TransitionControl>
            </div>
          )}
